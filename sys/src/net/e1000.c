...
static void e1000_rx_poll(void)
{
    ...
    while (guard++ < E1000_NUM_RX) {
        ...
        d->status = 0;
        /* RDT must name the LAST FREED descriptor (one behind the
         * next one the card may write), not the next read slot. */
        {
            uint32_t freed = rx_tail;
            rx_tail = (rx_tail + 1) % E1000_NUM_RX;
            e1000_write(E1000_RDT, freed);
        }
    }
}
